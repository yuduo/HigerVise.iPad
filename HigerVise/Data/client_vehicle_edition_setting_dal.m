...
@implementation client_vehicle_edition_setting_dal
...
+ (NSNumber *)getMaxId
{
    ...
    [db open];
    NSNumber *rtn;
    FMResultSet *result = [db executeQuery:@"SELECT MAX(vehicle_edition_setting_id) FROM client_vehicle_edition_setting"];
    if ([result next]) {
...
+ (BOOL)exists:(NSNumber *)vehicle_edition_setting_id
{
    ...
    [db open];
    BOOL rtn;
    FMResultSet *result = [db executeQuery:@"SELECT COUNT(vehicle_edition_setting_id) FROM client_vehicle_edition_setting WHERE vehicle_edition_setting_id = ?", vehicle_edition_setting_id];
    if ([result next]) {
...
+ (BOOL)add:(client_vehicle_edition_setting *)model
{
    ...
    [db open];
    BOOL rtn;
    rtn = [db executeUpdate:@"INSERT INTO client_vehicle_edition_setting (vehicle_edition_setting_id, vehicle_configurator_id, vehicle_edition_id, group_code, sale_group_code, op_code, op_name, sale_op_name, op_seq, op_value_code, op_value_name, sale_op_value_name, op_value_level_code, op_value_std_price, op_value_del_price, op_show_grade_disid, view_desc, is_displayed, is_displayed_config, is_allow_edited, is_has_image, is_selected, is_canceled, data_version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", model.vehicle_edition_setting_id, model.vehicle_configurator_id, model.vehicle_edition_id, model.group_code, model.sale_group_code, model.op_code, model.op_name, model.sale_op_name, model.op_seq, model.op_value_code, model.op_value_name, model.sale_op_value_name, model.op_value_level_code, model.op_value_std_price, model.op_value_del_price, model.op_show_grade_disid, model.view_desc, model.is_displayed, model.is_displayed_config, model.is_allow_edited, model.is_has_image, model.is_selected, model.is_canceled, model.data_version];
    [db close];
    return rtn;
}
...
+ (BOOL)update:(client_vehicle_edition_setting *)model
{
    ...
    [db open];
    BOOL rtn;
    rtn = [db executeUpdate:@"UPDATE client_vehicle_edition_setting SET vehicle_configurator_id = ?, vehicle_edition_id = ?, group_code = ?, sale_group_code = ?, op_code = ?, op_name = ?, sale_op_name = ?, op_seq = ?, op_value_code = ?, op_value_name = ?, sale_op_value_name = ?, op_value_level_code = ?, op_value_std_price = ?, op_value_del_price = ?, op_show_grade_disid = ?, view_desc = ?, is_displayed = ?, is_displayed_config = ?, is_allow_edited = ?, is_has_image = ?, is_selected = ?, is_canceled = ?, data_version = ? WHERE vehicle_edition_setting_id = ?", model.vehicle_configurator_id, model.vehicle_edition_id, model.group_code, model.sale_group_code, model.op_code, model.op_name, model.sale_op_name, model.op_seq, model.op_value_code, model.op_value_name, model.sale_op_value_name, model.op_value_level_code, model.op_value_std_price, model.op_value_del_price, model.op_show_grade_disid, model.view_desc, model.is_displayed, model.is_displayed_config, model.is_allow_edited, model.is_has_image, model.is_selected, model.is_canceled, model.data_version, model.vehicle_edition_setting_id];
    [db close];
    return rtn;
}
...
+ (BOOL)delete:(NSNumber *)vehicle_edition_setting_id
{
    ...
    [db open];
    BOOL rtn;
    rtn = [db executeUpdate:@"DELETE FROM client_vehicle_edition_setting WHERE vehicle_edition_setting_id = ?", vehicle_edition_setting_id];
    [db close];
    return rtn;
}
...
+ (BOOL)deleteList:(NSString *)where
{
    ...
    [db open];
    BOOL rtn;
    NSString *strSql = [NSString stringWithFormat:@"DELETE FROM client_vehicle_edition_setting WHERE %@", where];
...
+ (client_vehicle_edition_setting *)get:(NSNumber *)vehicle_edition_setting_id
{
    ...
    FMDatabase *db = [[FMDatabase alloc] initWithPath:dbPath];
    [db open];
    client_vehicle_edition_setting *rtn;
    FMResultSet *result = [db executeQuery:@"SELECT * FROM client_vehicle_edition_setting WHERE vehicle_edition_setting_id = ?", vehicle_edition_setting_id];
    while ([result next]) {
        rtn = [[client_vehicle_edition_setting alloc] init];
...
+ (NSArray *)getList:(NSString *)where
{
    ...
    NSString *strSql = [NSString stringWithFormat:@"SELECT * FROM client_vehicle_edition_setting WHERE %@", where];
    FMResultSet *result = [db executeQuery:strSql];
    while ([result next]) {
        client_vehicle_edition_setting *rtn = [[client_vehicle_edition_setting alloc] init];
        rtn.vehicle_edition_setting_id = [NSNumber numberWithInt:[result intForColumn:@"vehicle_edition_setting_id"]];
        rtn.vehicle_configurator_id = [NSNumber numberWithInt:[result intForColumn:@"vehicle_configurator_id"]];
        rtn.vehicle_edition_id = [NSNumber numberWithInt:[result intForColumn:@"vehicle_edition_id"]];
        rtn.group_code = [result stringForColumn:@"group_code"];
...
+ (NSArray *)getList:(NSString *)where order:(NSString *)order
{
    ...
    [db open];
    NSMutableArray *array = [[NSMutableArray alloc] init];
    NSString *strSql = [NSString stringWithFormat:@"SELECT * FROM client_vehicle_edition_setting WHERE %@ ORDER BY %@", where, order];
    FMResultSet *result = [db executeQuery:strSql];
    while ([result next]) {
        client_vehicle_edition_setting *rtn = [[client_vehicle_edition_setting alloc] init];
        rtn.vehicle_edition_setting_id = [NSNumber numberWithInt:[result intForColumn:@"vehicle_edition_setting_id"]];
...
+ (NSArray *)getList:(NSString *)where order:(NSString *)order top:(NSInteger)top
{
    ...
    [db open];
    NSMutableArray *array = [[NSMutableArray alloc] init];
    NSString *strSql = [NSString stringWithFormat:@"SELECT * FROM client_vehicle_edition_setting WHERE %@ ORDER BY %@ LIMIT 0,%d", where, order, top];
    FMResultSet *result = [db executeQuery:strSql];
    while ([result next]) {
        client_vehicle_edition_setting *rtn = [[client_vehicle_edition_setting alloc] init];
        rtn.vehicle_edition_setting_id = [NSNumber numberWithInt:[result intForColumn:@"vehicle_edition_setting_id"]];
...
+ (NSArray *)getList:(NSString *)where order:(NSString *)order beginIndex:(NSInteger)beginIndex length:(NSInteger)length
{
    ...
    [db open];
    NSMutableArray *array = [[NSMutableArray alloc] init];
    NSString *strSql = [NSString stringWithFormat:@"SELECT * FROM client_vehicle_edition_setting WHERE %@ ORDER BY %@ LIMIT %d,%d", where, order, beginIndex, length];
    FMResultSet *result = [db executeQuery:strSql];
    while ([result next]) {
        client_vehicle_edition_setting *rtn = [[client_vehicle_edition_setting alloc] init];
        rtn.vehicle_edition_setting_id = [NSNumber numberWithInt:[result intForColumn:@"vehicle_edition_setting_id"]];
...
{
    client_vehicle_edition_setting *model = [[client_vehicle_edition_setting alloc] init];
...
+ (NSArray *)getListForPriceTable:(NSString *)where
{
    ...
    [db open];
    NSMutableArray *array = [[NSMutableArray alloc] init];
    NSString *strSql = [NSString stringWithFormat:@"SELECT * FROM client_vehicle_edition_setting WHERE %@ ORDER BY op_seq ASC", where];
    FMResultSet *result = [db executeQuery:strSql];
    while ([result next]) {
